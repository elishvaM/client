...
export default function AddItem({setOpenAddItem}) {
  let { register, handleSubmit, getValues, formState: 
    { isValid, errors, dirtyFields, touchedFields, isDirty } } = useForm({ mode: "onTouched" });
...
<div  style={{display:'flex'}}>
        ...
        //??? מה לעשות אם בא לי רק את הבדיקת תקינות בלי השליחה
         {...register("Name", { minLength: 2, maxLength: 30, required: true })}
         onChange={(e)=> setName(e.target.value)}
        id="outlined-basic" label="שם הפריט" variant="outlined" sx={{margin:"8px 20px 0px 0px"}}/>
            {errors.Name && errors.Name.type == "required" &&
                <div className="error">
                    שם מוצר הוא שדה חובה
                </div>}
            {errors.Name?.type == "minLength" &&
                <div className="error">
                    שם מוצר לפחות 2 תויים
                </div>}
            {errors.Name?.type == "maxLength" &&
                <div className="error">
                    שם מוצר מקסימום 30 תוים
                </div>}
            </div>
          
        <FormControl variant="filled" sx={{ m: 1, minWidth: 120 }}>
        <InputLabel 
        id="demo-simple-select-filled-label">סוג הפריט</InputLabel>
        <Select
        ...
        >
         ...
         {productTypes.map(i => <MenuItem value={i.Id}>{i.Type}</MenuItem>)}
       ...
       <FormControl variant="filled" sx={{ m: 1, minWidth: 160 }}                >
        <InputLabel
        id="demo-simple-select-filled-label">סוג אחסון הפריט</InputLabel>
        ...
        >
          {/* <MenuItem value="">
           ...
           {storageTypes.map(i => <MenuItem value={i.Id}>{i.Type}</MenuItem>)}
          
        </Select>
      </FormControl>
      </div>
     <br/><br/>
      <FormControlLabel  sx={{position:'absolute', right:'1.8rem'}}
